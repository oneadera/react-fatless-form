...
describe('adaptTextField - value display', () => {
...
describe('adaptTextField - onChangeText', () => {
  it('is the exact same function reference as field.setValue - RN already hands back a plain string, nothing to unwrap', () => {
    const field = createFieldBinding<SignupValues, 'email'>()
    expect(adaptTextField(field).onChangeText).toBe(field.setValue)
  })

  it('behaviorally: calling onChangeText forwards the text straight to setValue', () => {
    const field = createFieldBinding<SignupValues, 'email'>()
    adaptTextField(field).onChangeText('a@b.com')
    expect(field.setValue).toHaveBeenCalledWith('a@b.com')
  })
})

describe('adaptTextField - passthrough', () => {
...
describe('useTextField', () => {
  it('reflects the initial form value', () => {
    const { result } = renderWithForm(() => useTextField<SignupValues>('email'), {
      email: 'a@b.com',
      address: { street: '' },
    })
  ...
  it('updates the underlying form value when onChangeText is called', () => {
    const { result, form } = renderWithForm(() => useTextField<SignupValues>('email'), {
      email: '',
      address: { street: '' },
    })

    act(() => {
      result.current.onChangeText('a@b.com')
    })
  ...
  it('reads and writes a nested field path', () => {
    const { result } = renderWithForm(() => useTextField<SignupValues>('address.street'), {
      email: '',
  ...
  it('onBlur marks the field touched', () => {
    const { result } = renderWithForm(() => useTextField<SignupValues>('email'), {
      email: '',
      address: { street: '' },
    })

...
- **`web` and `native` tests resolve `react-fatless-form` to core's source, not `dist/`.** Each package's `jest.config.cjs` maps it via `moduleNameMapper`, mirroring the `paths` alias already in that package's own `tsconfig.json`. This means `yarn test` never depends on `yarn build` having run first.
...
- **[examples/web](examples/web)** - [MUI](https://mui.com)
- **[examples/native](examples/native)** - [React Native Paper](https://callstack.github.io/react-native-paper/) via Expo
